...
function fold(groupId) {
    ...
    for(var i = 0; i < elementIds.length; i++){
        var thisElement = total.get(elementIds[i]);
        display.splice(display.indexOf(thisElement), 1); // Remove this element from display
    }

    thisGroup.width = thisGroup.name.length * ELEMENT_CH_WIDTH + PADDING * 2;
    thisGroup.y += PADDING_GROUP;
    thisGroup.height -= (2 * PADDING_GROUP);
    thisGroup.fold = true;
}
